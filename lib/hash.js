...
exports.hincrby = function (mockInstance, hash, key, increment, callback) {
    ...
    mockInstance.storage[hash].hash[key] = mockInstance.storage[hash].hash[key] || 0;

    mockInstance.storage[hash].hash[key] += increment;

    if (callback) {
        process.nextTick(function () {
            callback(null, mockInstance.storage[hash].hash[key]);
        });
    }
};
